Imports denaro
Partial Class inventory_tasklist
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            txtDateTo.Text = Format(vLastDate.AddDays(-1), "MM/dd/yyyy")
    ...
    Protected Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        ...
        Session.Remove("pMode")
        hDFrom.Value = txtDateFrom.Text.Trim
        hDTo.Value = txtDateFrom.Text.Trim
    End Sub
    ...
    Private Sub DataRefresh(pMode As String)
        ...
        vSQL = "select JobOrderNo,Alt_Cd,PurchaseOrderNo,Item_Cd,OrderQty,Bom_Cd,BomRev,TranId," _
            & "(select Descr from item_master where item_master.Item_Cd=jo_header.Item_Cd) as Item_Name," _
            & "(select Descr from ref_item_type where ref_item_type.Type_Cd=jo_header.ItemType_Cd) as vItemType," _
            & "(select Descr from ref_item_uom where ref_item_uom.UOM_Cd=jo_header.UOM_Cd) as vUom, " _
            & "(select sum(Qty) from prod_request_materials where JONO=JobOrderNo) as vItemRequest, " _
            & "(select sum(Qty) from prod_return_materials where JONO=JobOrderNo) as vItemReturn " _
            & "from jo_header where JobOrderNo is not null " & vFilter
    ...
    Protected Sub tbl_Tasklist_SelectedIndexChanged(sender As Object, e As EventArgs) Handles tbl_Tasklist.SelectedIndexChanged
        ...
        Session("pTranId") = tbl_Tasklist.SelectedRow.Cells(2).Text
        Session("pBOM") = tbl_Tasklist.SelectedRow.Cells(3).Text
        Session("pBOMRev") = tbl_Tasklist.SelectedRow.Cells(4).Text
        Session("pJO") = tbl_Tasklist.SelectedRow.Cells(5).Text

        Server.Transfer("taskdetails.aspx")
