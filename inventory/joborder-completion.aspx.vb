...
Partial Class inventory_jocomplition
    ...
    Private Sub CollectCompletion(pType As String)
		...
		vSQL = "select TranId,Qty,JONO,BatchNo, " _
			...
			& "TtlPcs, TtlPcsBox, " _
			& "(select count(TranId) from prod_sfgrewind  where prod_sfgrewind.CompletionTranId = prod_completion.TranId) as RewindCancel " _
        ...
        Try
			...
			Do While rs.Read
				...
				vPrevBatchNum = Regex.Replace(rs("PrevBatchNoA"), "[^A-Za-z0-9\-/]", " ")

				vData += "<td>" & vVoidBy & vDateVoid & "</td>" _
					& "<td>" _
					& "<div class='btn-group btn-group-sm' role='group' aria-label='...'> " _
					& "<Button type='button' class='btn btn-xs btn-success' data-toggle='modal' " _
					& "onclick='ModalEdit(" & rs("TranId") & ",""" & vPrevBatchNum & """,""" _
					& vCore & """,""" _
					& rs("Meter") & """,""" _
					& rs("Qty") & """)' " _
				...
				If Not IsDBNull(rs("DateVoid")) Then
					vData += "<Button type='button' class='btn btn-danger' data-toggle='modal' " _
				...
				If pType = "COMPLETION" Then
					...
					If rs("RewindCancel") = 0 Then
						vData += "<input type='button' id='" & rs("TranId") & "' name='" & rs("TranId") & "' class='btn btn-primary' " _
									& "onclick='ForRewind(" & rs("TranId") & ",this.value, """ & rs("JONO") & """)' " _
									& " value='For rewind'>"
					Else
						vData += "<input type='button' id='" & rs("TranId") & "' name='" & rs("TranId") & "' class='btn btn-danger' " _
									& "onclick='ForRewind(" & rs("TranId") & ",this.value, """ & rs("JONO") & """)' " _
									& " value='Cancel'>"
					End If
			...
			vData += "<tr style='font-weight:bold; color: #434343; background:#93c8ff;'>" _
				...
				& "<td colspan='4'></td>" _
			& "</tr><tr><td colspan='15'><h4 class='text-info'>WASTE</h4></td></tr>"
